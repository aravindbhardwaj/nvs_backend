...
export function validateOrganizationLeaderFile(
  ...
  const allowed = ALLOWED_ORGANIZATION_LEADER_IMAGE_TYPES[extension];
  const base = file.originalname.slice(0, -(extension.length + 1));
  const doubleExtension = Object.keys(
    ALLOWED_ORGANIZATION_LEADER_IMAGE_TYPES,
  ).some((item) => base.toLowerCase().endsWith(`.${item}`));
  if (
    !allowed ||
    doubleExtension ||
    ...
    throw new BadRequestException(
      'The uploaded organization leader picture type is not allowed.',
    );
  }
}

export async function validateOrganizationLeaderImage(
  file: Express.Multer.File,
): Promise<void> {
  ...
  if (!detectedType || !extensionMatches) {
    throw new BadRequestException(
      'The uploaded organization leader picture content is invalid.',
    );
  }
  file.mimetype = detectedType;
}

export const organizationLeadershipStorage = diskStorage({

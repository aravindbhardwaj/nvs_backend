...
const actor = { id: 10, organizationId: 5, role: Role.HEADQUARTER, email: 'user@nvs.gov.in' };
...
describe('BannersService', () => {
  ...
  it('rejects an end date before the start date before changing a banner', async () => {
    prisma.banner.findFirst.mockResolvedValue({
      id: 1, organizationId: 5, startDate: null, endDate: null,
    });
  ...
  it('enforces organization ownership before updates', async () => {
    prisma.banner.findFirst.mockResolvedValue({
      id: 1, organizationId: 6, startDate: null, endDate: null,
    });
    ownership.assertAccess.mockImplementation(() => { throw new ForbiddenException(); });

    await expect(service.update(1, {}, actor)).rejects.toBeInstanceOf(ForbiddenException);
    expect(prisma.$transaction).not.toHaveBeenCalled();
  });
  ...
  it('limits a non-super-admin management list to the actor organization', async () => {
    ...
    await service.findAll({ page: 1, limit: 20, sort: 'displayOrder', order: 'asc' }, actor);

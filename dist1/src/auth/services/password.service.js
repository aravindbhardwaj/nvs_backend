...
let PasswordService = class PasswordService {
    ...
    static PASSWORD_REGEX = /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&^#()_\-+=])[A-Za-z\d@$!%*?&^#()_\-+=]{12,128}$/;
    validate(password) {
        if (!PasswordService_1.PASSWORD_REGEX.test(password)) {
    ...
    async hashSecret(secret) {
        const rounds = this.configService.getOrThrow('auth.bcrypt.rounds');

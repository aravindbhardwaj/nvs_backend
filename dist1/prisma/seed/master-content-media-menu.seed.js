...
async function upsertMasterData(transaction) {
    ...
    return {
        contentTypesByCode: new Map(contentTypes.flatMap(({ id, code }) => (code ? [[code, id]] : []))),
        mediaTypesByCode: new Map(mediaTypes.flatMap(({ id, code }) => (code ? [[code, id]] : []))),
    };
}
async function upsertMenu(transaction, input) {
...
async function seedMenus(transaction, contentTypesByCode, mediaTypesByCode) {
    ...
    const organizationTypeIds = new Map(organizationTypes.map(({ id, code }) => [code, id]));
    const userId = superAdmin?.id ?? null;
    for (const [organizationTypeCode, headerMenus] of Object.entries(HEADER_MENU_SEEDS)) {
        const organizationTypeId = organizationTypeIds.get(organizationTypeCode);
        if (!organizationTypeId) {
            throw new Error(`Missing organization type required for menu seeding: ${organizationTypeCode}.`);
        }
        for (const [index, menu] of headerMenus.entries()) {
            ...
            if (menu.mediaTypeCode && !mediaTypeId) {
                throw new Error(`Missing media type required for menu seeding: ${menu.mediaTypeCode}.`);
            }
        ...
        for (const [index, menu] of FOOTER_MENU_SEEDS.entries()) {
            ...
            if (menu.contentTypeCode && !contentTypeId) {
                throw new Error(`Missing content type required for menu seeding: ${menu.contentTypeCode}.`);
            }
            await upsertMenu(transaction, {
                organizationTypeId,
                menuLocation: 2,
...
async function validateSeed(transaction) {
    ...
    if (contentCount !== TARGET_MASTER_NAMES.length ||
        mediaCount !== TARGET_MASTER_NAMES.length) {
        throw new Error(`Master-data validation failed: content=${contentCount}/${TARGET_MASTER_NAMES.length}, media=${mediaCount}/${TARGET_MASTER_NAMES.length}.`);
    }
}
async function main() {
    await prisma.$transaction(async (transaction) => {
        const { contentTypesByCode, mediaTypesByCode } = await upsertMasterData(transaction);
        await seedMenus(transaction, contentTypesByCode, mediaTypesByCode);
        await validateSeed(transaction);
    });
    console.log(`Seeded ${TARGET_MASTER_NAMES.length} Content Types, ${TARGET_MASTER_NAMES.length} Media Types, and 55 first-level menus.`);
}
main()
    .catch((error) => {
    console.error(error);
...
//# sourceMappingURL=master-content-media-menu.seed.js.map